...
export function calculateExpansionTool(slug: string, v: Values): CalculationResult {
  switch (slug) {
    ...
    case "afdd-selection-calculator": return afddSelection(v);
    case "dc-breaker-sizing-calculator": return dcBreakerSizing(v);
    case "pv-dc-isolator-sizing-calculator": return pvDcIsolator(v);
    case "acb-lsig-setting-calculator": return acbLsig(v);
    case "ct-ratio-burden-calculator": return ctRatioBurden(v);
    case "breaker-accessory-power-calculator": return breakerAccessoryPower(v);
    case "pv-dc-ats-calculator": return pvDcAts(v);
    ...
    case "solid-state-relay-calculator": return solidStateRelay(v);
...
function solidStateRelay(v: Values): CalculationResult {
  const mode = s(v.mode);
  if (mode === "select") {
    const system = s(v.system), voltage = pos(v.loadVoltage, "Load voltage"), pf = bounded(v.powerFactor, "Power factor", 0.01, 1), efficiency = bounded(v.efficiency, "Load efficiency", 1, 100) / 100;
    const current = s(v.inputMethod) === "current" ? pos(v.loadCurrent, "Load current") : pos(v.loadPower, "Load power") * 1000 / ((system === "three-ac" ? Math.sqrt(3) : 1) * voltage * (system === "dc" ? 1 : pf) * efficiency);
    const utilization = bounded(v.ratedUtilization, "Allowed datasheet current utilization", 1, 100) / 100, requiredRating = current / utilization;
    const voltageMargin = 1 + bounded(v.voltageMargin, "Voltage headroom", 0, 200) / 100, voltageScreen = voltage * voltageMargin, inrush = nonneg(v.expectedInrush, "Expected peak inrush"), control = pos(v.controlVoltage, "Control voltage"), loadType = s(v.loadType);
    const poles = system === "three-ac" ? 3 : 1;
    const output = system === "dc" ? "DC-output MOSFET / transistor SSR" : system === "three-ac" ? "Three-phase AC-output SSR" : "Single-phase AC-output SSR";
    const switching = system === "dc" ? "DC switching – zero-cross does not apply" : loadType === "heater" ? "Zero-cross starting preference" : loadType === "capacitive" || loadType === "lamp" ? "Zero-cross with inrush verification" : "Random / instantaneous turn-on application review";
    const loadNote = loadType === "heater" ? "Resistive heating is the clearest SSR duty, but element resistance and current still change with temperature." : loadType === "motor" ? "Motor starting, cycling, stall, reversing, and phase loss require an SSR specifically rated for motor duty." : loadType === "transformer" ? "Transformer energization can produce severe asymmetric inrush; verify the manufacturer's transformer-load rating and switching method." : loadType === "lamp" ? "Cold filament resistance can create high inrush even when steady current is modest." : loadType === "solenoid" ? "Inductive turn-off transients require the SSR manufacturer's suppression and dv/dt guidance." : "Capacitive inputs and LED drivers can have short, high charging pulses that govern surge selection.";
    return result(`≥ ${f(requiredRating)} A SSR screen`, "caution", `The entered load requires ${f(current)} A per conducting pole. At ${f(utilization * 100)}% of datasheet rating, the continuous-current screen is at least ${f(requiredRating)} A.`, [["Calculated load current", `${f(current)} A`], ["Output technology", output], ["Conducting poles", `${poles}`], ["Minimum current-rating screen", `${f(requiredRating)} A`], ["Entered rated-current utilization", `${f(utilization * 100)}%`], ["Minimum load-voltage range screen", `${f(voltageScreen)} V`], ["Expected inrush to verify", `${f(inrush)} A peak`], ["Switching method direction", switching], ["Control-input requirement", `Input range must contain ${f(control)} V`], ["Load application", readable(loadType)]], [loadNote, "Replace the entered utilization with the exact allowable-current curve value for ambient temperature, heatsink, spacing, orientation, and load category.", "Verify repetitive peak current, non-repetitive surge current, dv/dt, off-state leakage, minimum load, switching frequency, and the coordinated semiconductor fuse.", "An SSR is not a disconnecting or safe-isolation device; provide the required mechanical isolation and a design response to shorted-output failure."]);
  }

  if (mode === "thermal") {
    const technology = s(v.outputTechnology), current = pos(v.thermalCurrent, "Load current"), poles = integerAtLeast(v.poles, "Conducting poles", 1), devices = integerAtLeast(v.sharedDevices, "SSR modules", 1), duty = bounded(v.conductionDuty, "Conduction duty cycle", 0.1, 100) / 100;
    if (poles > 3) throw new Error("Conducting poles must be between 1 and 3.");
    const lossPerPole = technology === "dc-resistance" ? current ** 2 * nonneg(v.onResistance, "On resistance") / 1000 * duty : pos(v.onStateVoltage, "On-state voltage") * current * duty;
    const lossPerModule = lossPerPole * poles, totalLoss = lossPerModule * devices, ambient = num(v.thermalAmbient, "Heatsink air temperature"), maxJunction = num(v.maxJunction, "Maximum junction temperature"), rJc = nonneg(v.rJc, "RθJC"), rCs = nonneg(v.rCs, "RθCS"), installedRsa = nonneg(v.installedRsa, "RθSA");
    if (maxJunction <= ambient) throw new Error("Maximum junction temperature must be above heatsink air temperature.");
    const localRise = lossPerPole * (rJc + rCs), allowableSinkRise = maxJunction - ambient - localRise, maxRsa = allowableSinkRise / totalLoss, sinkRise = totalLoss * installedRsa, estimatedJunction = ambient + sinkRise + localRise, margin = maxJunction - estimatedJunction;
    const feasible = maxRsa > 0, installedPass = feasible && installedRsa <= maxRsa;
    return result(feasible ? `RθSA ≤ ${f(maxRsa)} °C/W` : "Thermal path is not feasible", !feasible || !installedPass ? "warning" : margin < 15 ? "caution" : "ok", feasible ? `The common heatsink must provide ${f(maxRsa)} °C/W or lower under the entered orientation and airflow. The entered ${f(installedRsa)} °C/W heatsink predicts approximately ${f(estimatedJunction)}°C junction temperature.` : "Junction-to-case and interface rise consume the full temperature budget before heatsink-to-air resistance is included.", [["Conduction loss per pole", `${f(lossPerPole)} W`], ["Loss per SSR module", `${f(lossPerModule)} W`], ["Total heatsink load", `${f(totalLoss)} W`], ["Local junction-to-sink rise", `${f(localRise)}°C`], ["Maximum permissible RθSA", feasible ? `${f(maxRsa)} °C/W` : "No positive value"], ["Entered heatsink RθSA", `${f(installedRsa)} °C/W`], ["Estimated heatsink rise", `${f(sinkRise)}°C`], ["Estimated junction temperature", `${f(estimatedJunction)}°C`], ["Junction-temperature margin", `${f(margin)}°C`], ["Thermal check", installedPass ? "Passes entered thermal model" : "Does not pass entered thermal model"], ["Panel heat input", `${f(totalLoss)} W`]], [technology === "ac-drop" ? "Use the exact manufacturer power-loss curve when the on-state characteristic is not adequately represented by one voltage-drop value." : "Use maximum RDS(on) at operating junction temperature, not the typical 25°C value.", "Select the heatsink from performance data for its actual orientation, natural or forced airflow, enclosure temperature, altitude, spacing, and thermal-interface material.", `Enter ${f(totalLoss)} W in the panel heat-loss or enclosure cooling calculation, together with other cabinet losses.`, "Verify mounting torque, flatness, thermal compound or pad, touch temperature, and protective earth requirements."]);
  }

  const actualCurrent = pos(v.actualCurrent, "Actual current"), allowedCurrent = pos(v.allowedCurrent, "Allowable current"), actualVoltage = pos(v.actualVoltage, "Applied voltage"), ratedVoltage = pos(v.ratedVoltage, "Rated voltage"), inrush = nonneg(v.inrushCurrent, "Inrush current"), surge = pos(v.surgeRating, "Surge-current rating"), pulseSeconds = nonneg(v.pulseDuration, "Pulse duration") / 1000, ssrI2t = pos(v.ssrI2t, "SSR I²t"), fuseI2t = nonneg(v.fuseI2t, "Fuse clearing I²t"), leakage = nonneg(v.offLeakage, "Off-state leakage"), minimumLoad = nonneg(v.minimumLoad, "Minimum load current"), actualMinimum = nonneg(v.actualMinimumLoad, "Smallest load current");
  const pulseI2t = inrush ** 2 * pulseSeconds, currentPass = actualCurrent <= allowedCurrent, voltagePass = actualVoltage <= ratedVoltage, surgePass = inrush <= surge, i2tPass = pulseI2t <= ssrI2t, fusePass = fuseI2t <= ssrI2t, minimumPass = actualMinimum >= minimumLoad;
  const checks = [currentPass, voltagePass, surgePass, i2tPass, fusePass, minimumPass], passed = checks.filter(Boolean).length, allPass = passed === checks.length;
  return result(`${passed}/${checks.length} entered checks pass`, allPass ? "caution" : "warning", `The candidate ${allPass ? "passes" : "does not pass"} the entered numerical screens. This does not replace the exact manufacturer load, surge, thermal, and protection coordination tables.`, [["Continuous-current check", currentPass ? "Pass" : "Fail"], ["Current margin", `${f((allowedCurrent / actualCurrent - 1) * 100)}%`], ["Load-voltage check", voltagePass ? "Pass" : "Fail"], ["Voltage headroom", `${f((ratedVoltage / actualVoltage - 1) * 100)}%`], ["Peak-surge check", surgePass ? "Pass" : "Fail"], ["Surge-current ratio", `${f(surge / Math.max(inrush, 0.0001))} × entered peak`], ["Rectangular pulse I²t estimate", `${f(pulseI2t)} A²s`], ["SSR I²t screen", i2tPass ? "Pass" : "Fail"], ["Fuse total-clearing I²t screen", fusePass ? "Pass" : "Fail"], ["Minimum-load check", minimumPass ? "Pass" : "Fail"], ["Maximum off-state leakage", `${f(leakage)} mA`]], ["Compare surge and I²t only on the same waveform, half-cycle or pulse duration, initial junction temperature, line frequency, and repetition basis stated in the SSR datasheet.", "Fuse total-clearing I²t below SSR I²t is only a first screen; use the SSR manufacturer's approved semiconductor-fuse coordination table and required safety margin.", "Verify thermal performance separately with the heatsink mode and the exact allowable-current derating curve.", "Assess whether off-state leakage can energize, glow, move, or shock the connected load; add a manufacturer-approved bleeder or mechanical isolation where required."]);
}
...
function afddSelection(v: Values): CalculationResult {
  const market = s(v.market), premises = s(v.premises), circuit = s(v.circuit), voltage = pos(v.voltage, "Circuit voltage"), designCurrent = pos(v.designCurrent, "Design current"), candidateCurrent = pos(v.candidateCurrent, "Candidate current"), curve = s(v.curve).toUpperCase(), sensitivity = pos(v.residualSensitivity, "Residual sensitivity"), faultCurrent = nonneg(v.faultCurrent, "Prospective fault current");
  const currentPass = candidateCurrent >= designCurrent && candidateCurrent <= 40, voltagePass = voltage <= 240, curvePass = ["B", "C"].includes(curve), sensitivityPass = [10, 30].includes(sensitivity), faultPass = faultCurrent <= 6;
  const productPass = currentPass && voltagePass && curvePass && sensitivityPass && faultPass;
  const riskReview = ["sleeping", "fire-risk", "residential"].includes(premises) ? "Elevated AFDD review priority" : "Project-specific AFDD review";
  return result(productPass ? "VAF1-40 / VAF3-40M candidate" : "Configuration review required", productPass ? "caution" : "warning", productPass ? `The entered ${candidateCurrent} A ${curve}-curve configuration fits the published VIOX AFDD rating envelope. Applicability still depends on the rules adopted for the project.` : "One or more entered values fall outside the published VIOX AFDD rating envelope.", [["Requirement status", "Review local rules"], ["Risk-screen direction", riskReview], ["Region / rules basis", readable(market)], ["Circuit application", readable(circuit)], ["Rated-current check", currentPass ? "Pass" : "Fail"], ["Voltage check", voltagePass ? "Pass" : "Fail"], ["Residual-current function", `Type A ${sensitivity} mA`], ["Overcurrent characteristic", `${curve} curve`], ["Breaking-capacity check", faultPass ? "6 kA rating exceeds entered fault current" : "Entered fault current exceeds 6 kA"], ["Pole arrangement", "1P+N"], ["Product envelope", productPass ? "Passes entered ratings" : "Does not pass entered ratings"]], ["Confirm whether AFDD protection is required or recommended under the current local rules and project specification.", "Verify circuit length, conductor, fault current, residual-current behavior, load compatibility, documentation, and exact ordered model.", "An AFDD does not make wiring defects, loose terminals, overload coordination, or maintenance checks unnecessary."]);
}

function dcBreakerSizing(v: Values): CalculationResult {
  const mode = s(v.inputMode), voltage = pos(v.systemVoltage, "Maximum circuit voltage"), load = mode === "power" ? pos(v.loadPower, "Load power") * 1000 / voltage : pos(v.loadCurrent, "Load current"), factor = bounded(v.designFactor, "Design factor", 100, 200) / 100, required = load * factor, recommended = next(required, breakerSizes), cable = pos(v.cableAmpacity, "Cable ampacity"), candidate = pos(v.candidateCurrent, "Candidate breaker current"), candidateVoltage = pos(v.candidateVoltage, "Candidate DC voltage"), fault = nonneg(v.faultCurrent, "Prospective DC fault current"), breaking = pos(v.breakingCapacity, "DC breaking capacity"), poles = integerAtLeast(v.poles, "Pole count", 1);
  const currentPass = candidate >= required && candidate <= cable, voltagePass = candidateVoltage >= voltage, breakingPass = breaking >= fault, allPass = currentPass && voltagePass && breakingPass;
  return result(`${recommended} A DC breaker`, allPass ? "caution" : "warning", `The operating current is ${f(load)} A and the entered design factor produces ${f(required)} A. The next standard current reference is ${recommended} A.`, [["Operating current", `${f(load)} A`], ["Required design current", `${f(required)} A`], ["Standard current reference", `${recommended} A`], ["Ib <= In <= Iz check", currentPass ? "Pass" : "Fail"], ["DC voltage check", voltagePass ? "Pass" : "Fail"], ["DC breaking-capacity check", breakingPass ? "Pass" : "Fail"], ["Candidate pole arrangement", `${poles} pole${poles === 1 ? "" : "s"}`], ["Combined rating check", allPass ? "Passes entered ratings" : "Does not pass entered ratings"]], ["Use only the exact breaker DC voltage and breaking ratings for the documented pole connection and polarity.", "Verify cable protection, ambient derating, enclosure temperature, source time constant, selectivity, isolation, and applicable PV or battery rules.", "A higher pole count does not automatically provide a higher voltage rating unless the manufacturer documents that series connection."]);
}

function pvDcIsolator(v: Values): CalculationResult {
  const voc = pos(v.moduleVoc, "Module Voc"), beta = num(v.vocCoefficient, "Voc temperature coefficient") / 100, minTemp = num(v.minimumTemperature, "Minimum cell temperature"), modules = integerAtLeast(v.seriesModules, "Series modules", 1), isc = pos(v.moduleIsc, "Module Isc"), strings = integerAtLeast(v.parallelStrings, "Parallel strings", 1), factor = bounded(v.currentFactor, "Current factor", 100, 200) / 100;
  const coldModuleVoc = voc * (1 + beta * (minTemp - 25)), coldArrayVoc = coldModuleVoc * modules, designCurrent = isc * strings * factor, recommended = next(designCurrent, breakerSizes), candidateVoltage = pos(v.candidateVoltage, "Candidate DC voltage"), candidateCurrent = pos(v.candidateCurrent, "Candidate current"), poles = integerAtLeast(v.poles, "Pole count", 1), topology = s(v.topology), duty = s(v.duty);
  const voltagePass = candidateVoltage >= coldArrayVoc, currentPass = candidateCurrent >= designCurrent, polePass = topology !== "floating" || poles >= 2, pass = voltagePass && currentPass && polePass;
  return result(`${recommended} A / >= ${f(coldArrayVoc)} V DC`, pass ? "caution" : "warning", `The cold-corrected array open-circuit voltage is ${f(coldArrayVoc)} V and the design current is ${f(designCurrent)} A.`, [["Cold module Voc", `${f(coldModuleVoc)} V`], ["Cold array Voc", `${f(coldArrayVoc)} V`], ["Required design current", `${f(designCurrent)} A`], ["Standard current reference", `${recommended} A`], ["Switching-duty direction", duty === "load" ? "Operational DC-PV load switching" : "Isolation after current removal"], ["Voltage-rating check", voltagePass ? "Pass" : "Fail"], ["Operational-current check", currentPass ? "Pass" : "Fail"], ["Conductor / pole screen", polePass ? "Pass" : "Review all live conductors"], ["Candidate check", pass ? "Passes entered ratings" : "Does not pass entered ratings"]], ["Confirm the exact IEC 60947-3 utilization category for the intended switching duty.", "Verify the manufacturer's pole-series connection, polarity, mounting orientation, temperature derating, enclosure, and safe isolation instructions.", "Use site minimum cell temperature and the module datasheet coefficient; nominal array voltage is not the governing voltage."]);
}

function acbLsig(v: Values): CalculationResult {
  const ib = pos(v.designCurrent, "Design current"), iz = pos(v.cableAmpacity, "Cable ampacity"), margin = bounded(v.longTimeMargin, "Long-time margin", 100, 150) / 100, shortMultiple = bounded(v.shortTimeMultiple, "Short-time pickup", 1, 15), instantMultiple = bounded(v.instantaneousMultiple, "Instantaneous pickup", 1, 20), groundFraction = bounded(v.groundFaultFraction, "Ground-fault pickup", 10, 100) / 100, downstream = nonneg(v.downstreamRating, "Downstream rating"), fault = nonneg(v.faultCurrent, "Fault current"), breaking = pos(v.candidateBreaking, "Breaking capacity");
  const targetIr = ib * margin, frameSizes = [630, 800, 1000, 1250, 1600, 2000, 2500, 3200, 4000, 5000, 6300], frame = next(Math.max(ib, targetIr), frameSizes), ir = Math.min(targetIr, iz, frame), isd = ir * shortMultiple, ii = ir * instantMultiple, ig = frame * groundFraction, cablePass = ib <= ir && ir <= iz, breakingPass = breaking >= fault, downstreamGap = downstream > 0 ? ir / downstream : 0;
  return result(`${frame} A frame`, cablePass && breakingPass ? "caution" : "warning", `The preliminary frame reference is ${frame} A and the entered long-time margin produces an Ir starting point of ${Math.round(ir)} A.`, [["Frame current reference", `${frame} A`], ["Long-time pickup Ir", `${Math.round(ir)} A`], ["Short-time pickup Isd", `${f(isd)} A`], ["Instantaneous pickup Ii", `${f(ii)} A`], ["Ground-fault pickup Ig", `${f(ig)} A`], ["Ib <= Ir <= Iz check", cablePass ? "Pass" : "Fail"], ["Ir / downstream rating", downstream > 0 ? `${f(downstreamGap)} x` : "Not entered"], ["Breaking-capacity check", breakingPass ? "Passes entered fault current" : "Fails entered fault current"], ["Time-delay settings", "Manufacturer curve study required"]], ["Treat Ir, Isd, Ii, and Ig as worksheet starting points only; select actual available settings from the exact trip unit.", "Coordinate long-time, short-time, instantaneous, and ground-fault delays with downstream curves, conductor thermal limits, source fault levels, and required clearing times.", "Ground-fault pickup and delay require the system earthing arrangement, sensor configuration, residual path, and applicable rules."]);
}

function ctRatioBurden(v: Values): CalculationResult {
  const primary = pos(v.primaryCurrent, "Primary current"), margin = bounded(v.ratioMargin, "Ratio margin", 100, 200) / 100, secondary = pos(v.secondaryCurrent, "Secondary current"), length = nonneg(v.leadLength, "Lead length"), area = pos(v.leadArea, "Lead cross-section"), meter = nonneg(v.meterBurden, "Meter burden"), relay = nonneg(v.relayBurden, "Relay burden"), other = nonneg(v.otherBurden, "Other burden"), rated = pos(v.ratedBurden, "Rated burden"), duty = s(v.use);
  const primaryRatios = [50, 75, 100, 150, 200, 250, 300, 400, 500, 600, 800, 1000, 1200, 1500, 2000, 2500, 3000, 4000, 5000], selected = next(primary * margin, primaryRatios), leadResistance = 2 * 0.0175 * length / area, leadBurden = secondary ** 2 * leadResistance, total = leadBurden + meter + relay + other, burdenPass = total <= rated, utilization = total / rated * 100;
  return result(`${selected}/${secondary} A`, burdenPass ? "caution" : "warning", `The reference ratio is ${selected}/${secondary} A. The complete secondary circuit presents ${f(total)} VA burden at rated secondary current.`, [["Primary ratio requirement", `${f(primary * margin)} A`], ["Selected standard ratio", `${selected}/${secondary} A`], ["Lead loop resistance", `${f(leadResistance)} ohm`], ["Lead burden", `${f(leadBurden)} VA`], ["Instrument and relay burden", `${f(meter + relay + other)} VA`], ["Total connected burden", `${f(total)} VA`], ["Rated burden utilization", `${f(utilization)}%`], ["Burden check", burdenPass ? "Pass" : "Fail"], ["CT duty", readable(duty)]], ["Verify the exact CT ratio, accuracy class, rated burden, frequency, insulation, and primary installation arrangement.", duty === "protection" ? "Complete a protection-CT saturation check using class, ALF or knee point, winding resistance, maximum fault current, X/R ratio, and relay requirements." : "Check metering accuracy at expected normal and minimum current, plus instrument security factor where applicable.", "Keep CT secondary wiring short and never open-circuit an energized secondary."]);
}

function breakerAccessoryPower(v: Values): CalculationResult {
  const voltage = pos(v.supplyVoltage, "Control supply voltage"), mode = s(v.operationMode), shuntQty = integerNonneg(v.shuntQuantity, "Shunt-trip quantity"), shuntVa = nonneg(v.shuntPickupVa, "Shunt-trip demand"), uvrQty = integerNonneg(v.uvrQuantity, "UV release quantity"), uvrPickup = nonneg(v.uvrPickupVa, "UV release pickup"), uvrHold = nonneg(v.uvrHoldVa, "UV release holding demand"), motorQty = integerNonneg(v.motorQuantity, "Motor-operator quantity"), motorVa = nonneg(v.motorOperatingVa, "Motor-operator demand"), other = nonneg(v.otherContinuousVa, "Other continuous load"), margin = bounded(v.designMargin, "Design margin", 100, 200) / 100;
  const tripEvent = shuntQty * shuntVa + uvrQty * uvrPickup, motorEvent = motorQty * motorVa, peak = mode === "simultaneous" ? tripEvent + motorEvent : Math.max(tripEvent, motorEvent), continuous = uvrQty * uvrHold + other, supplyVa = Math.max(peak, continuous) * margin, peakCurrent = peak / voltage, continuousCurrent = continuous / voltage;
  return result(`>= ${Math.ceil(supplyVa)} VA supply`, "caution", `The entered accessories require ${f(peak)} VA during the governing operating event and ${f(continuous)} VA continuously before design margin.`, [["Peak operating demand", `${f(peak)} VA`], ["Continuous demand", `${f(continuous)} VA`], ["Peak current at entered voltage", `${f(peakCurrent)} A`], ["Continuous current at entered voltage", `${f(continuousCurrent)} A`], ["Supply reference with margin", `${f(supplyVa)} VA`], ["Coincidence basis", mode === "simultaneous" ? "All entered operating loads" : "Largest credible event"], ["Control voltage", `${f(voltage)} V AC/DC`]], ["Use the exact pickup, inrush, holding, operating-time, and tolerance data for every accessory.", "Check the power supply or control transformer's short-duration overload curve, output voltage dip, conductor voltage drop, protective device, and battery autonomy where applicable.", "Confirm accessory series, frame, AC/DC type, mounting, contacts, and terminals against the exact breaker."]);
}

function pvDcAts(v: Values): CalculationResult {
  const mode = s(v.inputMode), voltage = pos(v.systemVoltage, "Maximum DC voltage"), loadCurrent = mode === "power" ? pos(v.loadPower, "Transferred power") * 1000 / voltage : pos(v.loadCurrent, "Transferred current"), factor = bounded(v.designFactor, "Continuous-current factor", 100, 200) / 100, required = loadCurrent * factor, recommended = next(required, breakerSizes), candidateCurrent = pos(v.candidateCurrent, "Candidate current"), candidateVoltage = pos(v.candidateVoltage, "Candidate voltage"), poles = integerAtLeast(v.poles, "Switched poles", 1), topology = s(v.topology), transition = s(v.transition), source = s(v.sourceType);
  const sourceLabel = source === "battery-pv" ? "Battery / PV sources" : source === "dual-battery" ? "Dual battery banks" : "Dual PV inputs";
  const currentPass = candidateCurrent >= required, voltagePass = candidateVoltage >= voltage, polePass = topology !== "floating" || poles >= 2, openPass = transition === "open", pass = currentPass && voltagePass && polePass && openPass;
  return result(`${recommended} A DC ATS`, pass ? "caution" : "warning", `The maximum transferred load is ${f(loadCurrent)} A and the entered continuous factor produces a ${f(required)} A operational-current requirement.`, [["DC source arrangement", sourceLabel], ["Transferred load current", `${f(loadCurrent)} A`], ["Required design current", `${f(required)} A`], ["Standard current reference", `${recommended} A`], ["Current-rating check", currentPass ? "Pass" : "Fail"], ["Voltage-rating check", voltagePass ? "Pass" : "Fail"], ["Pole / conductor screen", polePass ? "Pass" : "Review all live conductors"], ["Transition screen", openPass ? "Open transition" : "Engineered source paralleling required"], ["Candidate check", pass ? "Passes entered ratings" : "Does not pass entered ratings"]], ["Use only a transfer device explicitly rated for DC switching at the entered voltage, current, utilization category, and pole connection.", "Provide electrical and mechanical interlocking, source isolation, polarity control, and a defined response to loss or recovery of either source.", transition === "closed" ? "Do not parallel the entered DC sources without a purpose-designed synchronization, current-sharing, protection, and interconnection scheme." : "Verify break-before-make timing prevents unintended source interconnection and suits the connected load."]);
}
